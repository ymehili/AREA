...
async def initiate_service_connection(
    ...
    try:
        ...
        logger.exception(
            "Failed to initiate OAuth connection for provider %s", provider
        )
        raise HTTPException(
...
async def handle_service_connection_callback(
    ...
    try:
        ...
        logger.exception(
            "Unexpected error during OAuth callback for provider %s", provider
        )
        return RedirectResponse(
    ...
    try:
        ...
        if provider in OAuth2ProviderFactory.get_supported_providers():
            ...
            if (
                ...
                and connection.oauth_metadata.get("connection_type") == "api_key"
            ):
                ...
                if provider == "openai":
                    ...
                    if (
                        connection.oauth_metadata
                        and connection.oauth_metadata.get("connection_type")
                        == "rss_url"
                    ):
                        ...
                        test_result = {
                            ...
                            "feed_title": result.get("feed_info", {}).get(
                                "title", "Unknown Feed"
                            ),
...
async def add_api_key_connection(
    ...
    if provider == "openai":
        ...
        if not (
            ...
            or api_key.startswith("sk-svcacct-")
        ):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code != 200:
                    ...
                    if response.status_code == 401:
                        error_detail = (
                            "Authentication failed. Invalid OpenWeatherMap API key."
                        )
                    elif response.status_code == 429:
        ...
        if not (
            rss_feed_url.startswith("http://") or rss_feed_url.startswith("https://")
        ):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        try:
            ...
            error_detail = (
                "Failed to validate RSS feed. Please check the URL and try again."
            )
    ...
    existing_connection = get_service_connection_by_user_and_service(
        db, str(current_user.id), provider
    )
