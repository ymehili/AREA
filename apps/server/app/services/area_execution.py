...
class ExecutionEngine:
    ...
    async def execute_area_steps(self, area: Area, event: dict) -> bool:
        ...
        try:
            ...
            execution_log.step_details["completed_at"] = datetime.now(
                timezone.utc
            ).isoformat()
            self.db.commit()
    ...
    async def _execute_delay_step(
        self, area: Area, step: AreaStep, event: dict
    ) -> None:
        ...
        if unit == "seconds":
            ...
            delay_seconds = duration
            logger.warning(
                f"Unrecognized time unit '{unit}' for delay in area {area.id}, defaulting to seconds"
            )
    ...
    async def _execute_action_reaction_step(
        self, area: Area, step: AreaStep, event: dict
    ) -> None:
        ...
        if not step.service or not step.action:
            raise ValueError(
                f"Action/reaction step missing service or action: service={step.service}, action={step.action}"
            )
        ...
        if not handler:
            error_msg = (
                f"No handler found for {step.step_type} {step.service}.{step.action}"
            )
            logger.warning(
                error_msg,
                extra={
