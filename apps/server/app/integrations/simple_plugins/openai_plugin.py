...
def _get_openai_api_key(area: Area, db: Session) -> str:
    ...
    if not connection:
        raise OpenAIConnectionError(
            "OpenAI service connection not found. Please add your OpenAI API key."
        )
...
def chat_completion_handler(area: Area, params: dict, event: dict) -> None:
    ...
    try:
        ...
        event["openai.output_tokens"] = result.get("usage", {}).get(
            "completion_tokens", 0
        )
...
def text_completion_handler(area: Area, params: dict, event: dict) -> None:
    ...
    try:
        ...
        with httpx.Client(
            ...
            response = client.post(
                "https://api.openai.com/v1/completions", json=payload
            )
...
def image_generation_handler(area: Area, params: dict, event: dict) -> None:
    ...
    try:
        ...
        with httpx.Client(
            ...
            response = client.post(
                "https://api.openai.com/v1/images/generations", json=payload
            )
...
def content_moderation_handler(area: Area, params: dict, event: dict) -> None:
    ...
    try:
        ...
        with httpx.Client(
            ...
            response = client.post(
                "https://api.openai.com/v1/moderations", json=payload
            )
