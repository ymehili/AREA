"""Google Drive OAuth2 provider implementation."""
...
class GoogleDriveOAuth2Provider(OAuth2Provider):
    """Google Drive OAuth2 provider implementation using Google OAuth."""
    ...
    def get_authorization_url(self, state: str, **kwargs) -> str:
        """Generate Google OAuth authorization URL for Drive."""
        params = {
    ...
    async def exchange_code_for_tokens(self, code: str, **kwargs) -> OAuth2TokenSet:
        """Exchange authorization code for Google Drive access and refresh tokens."""
        async with httpx.AsyncClient() as client:
            try:
                ...
                if "error" in data:
                    raise OAuth2TokenExchangeError(f"Google Drive OAuth error: {data['error']}")
    ...
    async def refresh_tokens(self, refresh_token: str) -> OAuth2TokenSet:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if "error" in data:
                    raise OAuth2RefreshError(f"Google Drive token refresh error: {data['error']}")
    ...
    async def test_api_access(self, access_token: str) -> Dict[str, Any]:
        """Test Google Drive API access with the token."""
        async with httpx.AsyncClient() as client:
            try:
                ...
                # List some files to verify full API access
                ...
                return {
                    "user": {
                        "email": about_data.get("user", {}).get("emailAddress"),
                        "display_name": about_data.get("user", {}).get("displayName"),
                    },
                    "storage": {
                        "limit": about_data.get("storageQuota", {}).get("limit"),
                        "usage": about_data.get("storageQuota", {}).get("usage"),
                    },
                    "files": [
                        {
                            "id": file.get("id"),
...
__all__ = ["GoogleDriveOAuth2Provider"]
